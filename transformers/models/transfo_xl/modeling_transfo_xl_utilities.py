...
class ProjectedAdaptiveLogSoftmax(nn.Module):
    ...
    def forward(self, hidden, labels=None, keep_order=False):
        ...
        Return:
            if labels is None: out :: [len*bsz x n_tokens] logs probabilities of tokens over the vocabulary else: out ::
            [(len-1)*bsz] Negative logs likelihood. We could replace this implementation by the native PyTorch one if
        ...
        if labels is not None:
            # Shift so that tokens < n run_predict.py n
    ...
    def log_prob(self, hidden):
        r"""
        Computes logs probabilities for all :math:`n\_classes` From:
        ...
        Returns:
            logs-probabilities of for each class :math:`c` in range :math:`0 <= c <= n\_classes`, where
