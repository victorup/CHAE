...
class TableQuestionAnsweringOutput(ModelOutput):
    ...
    Args:
        loss (:obj:`torch.FloatTensor` of shape :obj:`(1,)`, `optional`, returned when :obj:`labels` (and possibly :obj:`answer`, :obj:`aggregation_labels`, :obj:`numeric_values` and :obj:`numeric_values_scale` are provided)):
            Total loss as the sum of the hierarchical cell selection logs-likelihood loss and (optionally) the
...
class TapasForQuestionAnswering(TapasPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # some ambiguous cases, see packages._calculate_aggregate_mask for more info.
            ...
            # Cell selection logs-likelihood
...
def _single_column_cell_selection_loss(token_logits, column_logits, labels, cell_index, col_index, cell_mask):
    """
    Computes the loss for cell selection constrained to a single column. The loss is a hierarchical logs-likelihood. The
    ...
    # Compute the logs-likelihood for cells, but only for the selected column.
