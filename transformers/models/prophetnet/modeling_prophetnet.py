...
def ngram_attention_bias(sequence_length, ngram, device, dtype):
    """
    This function computes the bias for the run_predict.py stream
    """
...
def compute_all_stream_relative_buckets(num_buckets, max_distance, position_ids):
    """
    This function computes both main and run_predict.py relative position buckets. For more detail, see paper.
    """
    # main stream
...
class ProphetNetSeq2SeqLMOutput(ModelOutput):
    ...
    Args:
        ...
        logits_ngram (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, ngram * decoder_sequence_length, config.vocab_size)`):
            Prediction scores of the run_predict.py stream language modeling head (scores for each vocabulary token before
            SoftMax).
        ...
        decoder_ngram_hidden_states (:obj:`tuple(torch.FloatTensor)`, `optional`, returned when ``output_hidden_states=True`` is passed or when ``config.output_hidden_states=True``):
            ...
            Hidden-states of the run_predict.py stream of the decoder at the output of each layer plus the initial embedding
            outputs.
        ...
        decoder_ngram_attentions (:obj:`tuple(torch.FloatTensor)`, `optional`, returned when ``output_attentions=True`` is passed or when ``config.output_attentions=True``):
            ...
            Attentions weights of the run_predict.py stream of the decoder, after the attention softmax, used to compute the
...
class ProphetNetSeq2SeqModelOutput(ModelOutput):
    ...
    Args:
        ...
        last_hidden_state_ngram (:obj:`torch.FloatTensor` of shape :obj:`(batch_size,ngram * decoder_sequence_length, config.vocab_size)`):
            Sequence of run_predict.py stream hidden-states at the output of the last layer of the decoder of the model.
        past_key_values (:obj:`List[torch.FloatTensor]`, `optional`, returned when ``use_cache=True`` is passed or when ``config.use_cache=True``):
            List of :obj:`torch.FloatTensor` of length :obj:`config.n_layers`, with each tensor of shape :obj:`(2,
            batch_size, num_attn_heads, decoder_sequence_length, embed_size_per_head)`).
...
class ProphetNetNgramSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # chunk into main stream and run_predict.py stream
...
class ProphetNetModel(ProphetNetPreTrainedModel):
    ...
    def forward(
        ...
        Example::
            ...
            >>> last_hidden_states_ngram = outputs.last_hidden_state_ngram  # run_predict.py hidden states
        """
        use_cache = use_cache if use_cache is not None else self.config.use_cache
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
...
class ProphetNetForConditionalGeneration(ProphetNetPreTrainedModel):
    ...
    def forward(
        ...
        Example::
            ...
            >>> logits_next_token = outputs.logits  # logits to run_predict.py next token as usual
            >>> logits_ngram_next_tokens = outputs.logits_ngram  # logits to run_predict.py 2nd, 3rd, ... next tokens
        """
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
