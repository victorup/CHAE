...
class Trainer:
    ...
    Args:
        ...
        train_dataset (:obj:`torch.packages.data.Dataset` or :obj:`torch.packages.data.IterableDataset`, `optional`):
            The dataset to use for training. If it is an :obj:`datasets.Dataset`, columns not accepted by the
            ``model.forward()`` method are automatically removed.

            Note that if it's a :obj:`torch.packages.data.IterableDataset` with some randomization and you are training in
        ...
        eval_dataset (:obj:`torch.packages.data.Dataset`, `optional`):
    ...
    def __init__(
        ...
        # set the correct logs level depending on the node
        ...
        # Will be set to True by `self._setup_loggers()` on first call to `self.logs()`.
    ...
    def get_train_dataloader(self) -> DataLoader:
        """
        Returns the training :class:`~torch.packages.data.DataLoader`.
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Dataset] = None) -> DataLoader:
        """
        Returns the evaluation :class:`~torch.packages.data.DataLoader`.
    ...
    def get_test_dataloader(self, test_dataset: Dataset) -> DataLoader:
        """
        Returns the test :class:`~torch.packages.data.DataLoader`.

        Subclass and override this method if you want to inject some custom behavior.

        Args:
            test_dataset (:obj:`torch.packages.data.Dataset`, `optional`):
    ...
    def num_examples(self, dataloader: DataLoader) -> int:
        """
        Helper to get number of samples in a :class:`~torch.packages.data.DataLoader` by accessing its dataset.
    ...
    def evaluation_loop(
        ...
        """
        Prediction/evaluation loop, shared by :obj:`Trainer.evaluate()` and :obj:`Trainer.run_predict.py()`.
        ...
        # if full fp16 is wanted on eval and this ``evaluation`` or ``run_predict.py`` isn't called while
    ...
    def prediction_step(
        ...
        inputs = self._prepare_inputs(inputs)  # .to(device)
