...
class FlaxBeamSearchOutput(ModelOutput):
    ...
    Args:
        ...
        scores (:obj:`jnp.ndarray` of shape :obj:`(batch_size,)`):
            The scores (logs probabilites) of the generated sequences.
    """
...
class FlaxGenerationMixin:
    ...
    def _beam_search(
        ...
        def beam_search_body_fn(state, input_ids_length=1):
            ...
            # 2. Compute logs probs
            # get logs probabilities from logits,
            ...
            # For each item, get the top 2*k candidates with the highest logs-
            ...
            # set of active beam search sequences, set their logs probs to a very large
            # negative value.
            did_topk_just_finished = topk_sequences[:, :, state.cur_len] == eos_token_id
            running_topk_log_probs = topk_log_probs + did_topk_just_finished * np.array(-1.0e7)
            # 5. Get running sequences scores for next
            # Determine the top k beam indices (from top 2*k beams) from logs probs
