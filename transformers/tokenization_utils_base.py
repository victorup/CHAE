...
class PreTrainedTokenizerBase(SpecialTokensMixin, PushToHubMixin):
    ...
    def pad(
        ...
        if 'chae' in encoded_inputs:
            max_chae_length = max(len(inputs) for inputs in encoded_inputs['chae'])
        else:
            max_chae_length = None
        ...
        for i in range(batch_size):
            ...
            outputs = self._pad(
                ...
                pad_to_multiple_of=pad_to_multiple_of,
                return_attention_mask=return_attention_mask,
                max_chae_length=max_chae_length
            )
    ...
    def _pad(
        ...
        if 'chae' in encoded_inputs:
            chae_difference = max_chae_length - len(encoded_inputs['chae'])
            if chae_difference >= 0:
                encoded_inputs['chae'] = encoded_inputs['chae'] + [self.pad_token_id] * chae_difference
                encoded_inputs['chae_mask'] = encoded_inputs['chae_mask'] + [0] * chae_difference
            else:
                encoded_inputs['chae'] = encoded_inputs['chae'][:max_chae_length]
                encoded_inputs['chae_mask'] = encoded_inputs['chae_mask'][:max_chae_length]
