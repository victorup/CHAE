...
class BeamSearchDecoderOnlyOutput(ModelOutput):
    ...
    Args:
        ...
        scores (:obj:`tuple(torch.FloatTensor)` `optional`, returned when ``output_scores=True`` is passed or when ``config.output_scores=True``):
            Processed beam scores for each vocabulary token at each generation step. Beam scores consisting of logs
            softmax scores for each vocabulary token and sum of logs softmax of previously generated tokens in this beam
...
class GenerationMixin:
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        num_beams = num_beams if num_beams is not None else self.config.num_beams  # self.config.num_beams默认是4
        num_beam_groups = num_beam_groups if num_beam_groups is not None else self.config.num_beam_groups  # 默认1
        do_sample = do_sample if do_sample is not None else self.config.do_sample  # 默认False
        num_return_sequences = (
            num_return_sequences if num_return_sequences is not None else self.config.num_return_sequences  # 默认1
        )

        pad_token_id = pad_token_id if pad_token_id is not None else self.config.pad_token_id  # 默认1
        bos_token_id = bos_token_id if bos_token_id is not None else self.config.bos_token_id  # 默认0
        eos_token_id = eos_token_id if eos_token_id is not None else self.config.eos_token_id  # 默认2

        output_scores = output_scores if output_scores is not None else self.config.output_scores  # 默认False
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions  # 默认False
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states  # 默认False
        )
        return_dict_in_generate = (
            return_dict_in_generate if return_dict_in_generate is not None else self.config.return_dict_in_generate  # 默认False
        )
        ...
        src_input_ids = input_ids.index_select(0, expanded_return_idx)  # 为了保留输入的input_ids，在copy时用；beam操作为了和后面维度匹配
        ...
        if self.config.is_encoder_decoder:
            ...
            model_kwargs = self._prepare_encoder_decoder_kwargs_for_generation(input_ids, model_kwargs)  # 经过encoder <----------

            # set input_ids as decoder_input_ids
            if "decoder_input_ids" in model_kwargs:
                input_ids = model_kwargs.pop("decoder_input_ids")
            else:
                input_ids = self._prepare_decoder_input_ids_for_generation(  # input_ids改写  <----------
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # model_inputs = self.prepare_inputs_for_generation(input_ids, chae, chae_mask, src_input_ids, **model_kwargs)
            model_inputs = self.prepare_inputs_for_generation(input_ids, chae, chae_mask, chae, **model_kwargs)  # 只copy chae <----------04051820

            # forward pass to get next token
            outputs, emo_output = self(
                **model_inputs,
                return_dict=True,
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # prepare model inputs
            # model_inputs = self.prepare_inputs_for_generation(input_ids, chae, chae_mask, src_input_ids, **model_kwargs)  # copy所有
            model_inputs = self.prepare_inputs_for_generation(input_ids, chae, chae_mask, chae, **model_kwargs)  # 只copy chae <----------04051820
            ...
            next_token_logits = outputs.logits[:, -1, :]  # outputs.logits: (batch, 1, vocab), outputs.logits[:, -1, :] -> (batch, vocab)
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, chae, chae_mask, src_input_ids, **model_kwargs)  # transformers/models/bart/modeling_bart.py 1466

            outputs, emo_output = self(  # 进入模型
                **model_inputs,
                return_dict=True,
                output_attentions=output_attentions,
                output_hidden_states=output_hidden_states,
        ...
        if return_dict_in_generate:
            ...
            return sequence_outputs["sequences"], emo_output
