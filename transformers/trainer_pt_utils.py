...
class DistributedSamplerWithLoop(DistributedSampler):
    """
    Like a :obj:torch.packages.data.distributed.DistributedSampler` but loops at the end back to the beginning of the
    shuffled samples to make each process have a round multiple of batch_size samples.

    Args:
        dataset (:obj:`torch.packages.data.Dataset`):
...
class SequentialDistributedSampler(Sampler):
    ...
    Even though we only use this sampler for eval and run_predict.py (no training), which means that the model params won't
...
class IterableDatasetShard(IterableDataset):
    ...
    Args:
        dataset (:obj:`torch.packages.data.IterableDataset`):
...
def metrics_format(self, metrics: Dict[str, float]) -> Dict[str, float]:
    ...
    Args:
        metrics (:obj:`Dict[str, float]`):
            The metrics returned from train/evaluate/run_predict.py
...
def log_metrics(self, split, metrics):
    ...
    **Understanding the reports:**
    ...
    ``__init__``, ``train``, ``evaluate`` and ``run_predict.py`` calls.
    ...
    level of ``train``, ``evaluate`` and ``run_predict.py`` methods. Which means that if ``eval`` is called during ``train``,
