...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=True,
        help="Whether to use emotion classifier.",
    )
    parser.add_argument(
        "--do_emo_weight",
        type=str,
        default=True,
        help="Whether to use weighted cross entropy for emotion classifier.",
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
    ...
    parser.add_argument(
        "--train_file", type=str, default='data/n_to_1_new/emo_data/train.csv',
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="True for sampling or False for greedy/beam search",
    )
    parser.add_argument(
        "--top_k",
        type=int,
    ...
    parser.add_argument(
        "--validation_file", type=str, default='data/n_to_1_new/emo_data/valid.csv',
        help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
        "--test_file", type=str, default='data/n_to_1_new/emo_data/test.csv',
        help="A csv or a json file containing the test data."
    )
    parser.add_argument(
        "--pre_file", type=str, default='data/n_to_1_new/emo_data/pre.csv',
    ...
    parser.add_argument(
        "--model_name_or_path",
        type=str,
        default='checkpoint/n_to_1_new/04061730_n_to_1_new_epoch0',
        # default='checkpoint/n_to_1_new/04062200_n_to_1_new_epoch0',
    ...
    parser.add_argument("--num_train_epochs", type=int, default=3,  # 3
    ...
    parser.add_argument("--do_predict", default=False, help="Whether predict 1 to 1.")
    parser.add_argument("--do_eval_story", default=True, help="Whether predict 1 to n.")
    parser.add_argument("--do_self_predict", default=False, help="Whether predict on any input sentences.")
    parser.add_argument("--do_base_or_finetune", default='finetune', help="Whether predict on any input sentences.")
...
def eval_story(dataloader, prediction_file=None, label_file=None, num=None):
    ...
    gen_kwargs = {
        ...
        "num_beams": args.num_beams,  # num_beams=1: greedy search / top-k
        "do_sample": args.do_sample,  # beam_search = False / top-k = True
        "top_k": args.top_k,  # 50 / 30
        "temperature": args.temperature,  # 0.6 / 0.8 / 1 / 1.2
    }
    story_pre_list = []
    story_labels_list = []
    story_chae_list = []
...
def main():
    global model, tokenizer, postprocess_text, metric, config, model, train_dataloader, \
    ...
    file_handler = logging.FileHandler(filename='logs/log.txt')
    ...
    config.do_emo = args.do_emo
    config.emo_weight = args.do_emo_weight
    ...
    # add special tokens
    special_tokens_dict = {'additional_special_tokens': ["<SEP>", "<soc>", "<sep>", "<soa>", "<soe>", "<no_action>"]}
    ...
    if args.do_self_predict:
        ...
        logger.info('Run checkpoint : {}'.format(args.model_name_or_path))
        predict_epoch = args.model_name_or_path.split('_')[-1][5:]
        name = args.model_name_or_path.split('/')[-1]
        prediction_file = f'{name}_self_pre.txt'
        label_file = f'{name}_labels.txt'
    ...
    if args.do_predict:
        ...
        predict_epoch = args.model_name_or_path.split('_')[-1][5:]
        name = args.model_name_or_path.split('/')[-1]
        prediction_file = f'{name}_predictions.txt'
        label_file = f'{name}_labels.txt'
    ...
    if args.do_eval_story:
        ...
        name = args.model_name_or_path.split('/')[-1]
        if args.do_sample:
            prediction_file = f'{name}_topk{args.top_k}_temp{str(args.temperature).replace(".", "")}_story.txt'
        else:
            prediction_file = f'{name}_beam{args.num_beams}_story.txt'
        label_file = f'labels_story.txt'
        eval_bar = tqdm(range(len(test_dataloader)), disable=not accelerator.is_local_main_process,
                        desc='Eval (epoch #{})'.format(predict_epoch))
