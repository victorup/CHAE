...
def display_attention(sentence, translation, attention, pic_name):
    ...
    # save_dir = 'pic_attention/b{}_layer{}/'.format(pic_name[0], pic_name[1])
    save_dir = 'pic_attention/{}/'.format(time)
    ...
    for step, batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            generated_tokens, emo_output = accelerator.unwrap_model(model).generate(  # transformers/generation_utils.py 1789
                ...
                attention_mask=torch.cat([attention_mask, batch["chae_mask"]], 1) if batch["chae"] is not None else attention_mask,
            ...
            # de_labels = [''.join(label).strip() for label in decoded_labels]  #   <-------写labels
            ...
            tokenized_inputs = tokenizer(new_inputs,  max_length=args.max_target_length, padding=False, truncation=True, return_tensors='pt')  # 变tensor
            input_ids = tokenized_inputs['input_ids'].to(batch["input_ids"].device)  # 更新input_ids送入模型
            attention_mask = tokenized_inputs['attention_mask'].to(batch["input_ids"].device)  # 更新attention_mask送入模型
            # new_labels += ' ' + de_labels[0]  #  <-------写labels
            # new_chae.append(decoded_chae[0])  #  <-------写可控性需要chae
...
def train(completed_steps, epoch):
    model.train()
    # tokenizer: bos:{'<s>':0}, eos:{'</s>':2}, pad:{'<pad>':1}
    # config.bos_token_id:0, eos_token_id:2, pad_token_id:1, config.decoder_start_token_id:2
    # -100? 是干嘛用的
    for step, batch in enumerate(train_dataloader):
        ...
        if step % 100 == 0:
            if emo_output is None:
                ...
                # logger.info("epoch {} lm_loss {}".format(epoch, loss.item()-emo_output[2].item(), emo_output[2].item()))
            else:
                # logger.info("epoch {} loss {} emo_acc {}".format(epoch, loss.item(), emo_output[-1]))
                logger.info("epoch {} lm_loss {} emo_loss {} emo_acc {}".format(epoch, loss.item()-emo_output[2].item(), emo_output[2].item(), emo_output[-1]))
...
def evaluate(dataloader, prediction_file=None, label_file=None, num=None):
    ...
    for step, batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            if args.predict_with_generate or do_show_attention:
                generated_outputs, emo_output = accelerator.unwrap_model(model).generate(  # transformers/generation_utils.py 1789
                ...
                if do_show_attention:
                    ...
                    layer = -1  # 取attention第几层
                    # h = 15  # 取bbatch的第几个头
                    input_all = torch.cat([batch["input_ids"][b], batch["chae"][b]], 0)  # 0表示batch
                    mask_all = torch.cat([batch["attention_mask"][b], batch["chae_mask"][b]])
                    ...
                    # generated_outputs.cross_attentions: 返回pre_len个12层的[batch, head_num, 1, src_len]
                    # 取第0个pre的最后一层的头的和sum(1)的第0个batch: generated_outputs.cross_attentions[0][-1].sum(1)[0] :(1,86)
                    # 把每个pre的拼接起来，变为(pre_len, src_len)的attention
                    pre_list = []
                    pl = 0
                    for pre in generated_outputs.cross_attentions:  # generated_outputs.cross_attentions: 56个pre
                        ...
                        # pre_line = pre[layer].sum(1)[b]  # (1,86)  # 对所有head求和取第b个batch
                        # pre_line = pre[layer][b][h]  # (1,86)  # 取第h个头
                        # pre_line = pre[layer][b]  # (head_num, 1, 86)  # 展示所有头
                        # pre_list.append(torch.masked_select(pre_line, mask_all.bool()).reshape(pre_line.size(0), len(input_token)).unsqueeze(0))  # 保留所有头
                    ...
                    # pic_name = (b, layer, pic_num)
                    pic_name = (b, pic_num)
                    display_attention(input_token, pre_token, attention, pic_name)
                    pic_num += 1

                    # 展示每个头
                    # attention = attention.transpose(0, 1)  # (head_num, pre_len, src_len)
                    # for h in range(attention.size(0)):
                    #     pic_name = (b, layer, pic_num)
                    #     display_attention(input_token, pre_token, attention[h], pic_name)
                    #     pic_num += 1
    ...
    if emo_output is None:
        ...
        # logger.info({"valid_average_loss": valid_average_loss, "ppl": ppl, "bleu": eval_metric["score"], "emo_acc": valid_average_emo_acc})
        logger.info({"valid_average_loss": valid_average_loss, 'valid_average_emo_loss': valid_average_emo_loss, "ppl": ppl, "emo_acc": valid_average_emo_acc})
...
def main():
    ...
    for model_name_or_path in model_name_or_path_list:
        ...
        special_tokens_dict = {'additional_special_tokens': ["<SEP>", "<soc>", "<sep>", "<soa>", "<soe>", "<no_action>"]}  # <------- 04041620
        ...
        def preprocess_function_chae(examples):
            ...
            # 在每个inputs前添加<s>
            new_inputs = []  # <-----04042324
            for i in inputs:  # <-----04042324
                new_inputs.append(tokenizer.bos_token + ' ' + i)  # <-----04042324
            model_inputs = tokenizer(new_inputs, max_length=args.max_source_length, padding=padding, truncation=True, add_special_tokens=False)  # <-----04042324
            # '''
            # model_inputs = tokenizer(inputs, max_length=args.max_source_length, padding=padding, truncation=True, add_special_tokens=False)  # <------- 添加add_special_tokens=False
            ChAE = []
            emo_label = []
            # raise KeyError
            for i in range(len(characters)):
                ...
                for char in eval(characters[i]):
                    ...
                    for sent_a in eval(actions[i]):
                        if sent_a[char] == []:
                            a += '<no_action>' + ' '  # <------- 添加<no_action>
                        else:
                            a += ' <sep> '.join(sent_a[char]) + ' '  # <------- 添加<sep>
                    e = ''
                    ...
                    chae += '<SEP>' + ' <soc> ' + char + ' <soa> ' + a + '<soe> ' + e  # <------- 去掉<eoc>, <eoa>, <eoe>
                chae += tokenizer.eos_token  # <-----04042324
            ...
            model_ChAE = tokenizer(ChAE, max_length=args.max_source_length, padding=padding, truncation=True, add_special_tokens=False)  # <------- 添加add_special_tokens=False
            model_inputs['chae'] = model_ChAE["input_ids"]
            model_inputs['chae_mask'] = model_ChAE["attention_mask"]
            labels = tokenizer(targets, max_length=args.max_target_length, padding=padding, truncation=True, add_special_tokens=False)  # <------- 添加add_special_tokens=False
        ...
        if do_self_predict:
            ...
            if do_base_or_finetune == 'finetune':
                prediction_file = '{}_epoch{}_self_pre_beam2.txt'.format(name, predict_epoch)  # 根据相应的num_train_epochs或base来设置名称
        ...
        if do_eval_story:
            ...
            predict_epoch = model_name_or_path.split('_')[-1][5:]
            if do_base_or_finetune == 'finetune':
                prediction_file = '{}_epoch{}_story_topk50_temp12.txt'.format(name, predict_epoch)  # 根据相应的num_train_epochs或base来设置名称
                # prediction_file = '{}_epoch{}_story_greedy.txt'.format(name, predict_epoch)  # 根据相应的num_train_epochs或base来设置名称
...
if __name__ == "__main__":
    ...
    if do_predict or do_self_predict or do_eval_story or do_show_attention:
        # fine-tune
        if do_base_or_finetune == 'finetune':
            if do_predict:
